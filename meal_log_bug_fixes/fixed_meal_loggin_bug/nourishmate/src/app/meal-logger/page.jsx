...
// Debounce helper function outside component
...
export default function MealLoggerPage() {
  ...
  const fetchMeals = useCallback(async () => {
    ...
    // Prevent duplicate fetches in development mode (React 18 strict mode)
    setIsLoading(true);
    try {
  ...
  // Create a properly debounced version of fetchMeals that won't cause infinite loops
  ...
  useEffect(() => {
    if (userData && isAuthChecked) {
      // Only fetch if we haven't already or if the date changed
      if (!dataFetchedRef.current || date) {
        fetchMeals();
  ...
  // Handle visibility and focus events - SEPARATE from the data fetching effect
  useEffect(() => {
    if (!userData) return;
  ...
  }, [userData, debouncedFetchMeals]); // Only depends on userData and debouncedFetchMeals
  ...
  const handleAddMeal = async () => {
    ...
    fetchMeals(); // Use direct fetch, not debounced
  };
